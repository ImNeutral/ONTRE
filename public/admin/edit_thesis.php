...
*/
			...
			if($editable && isset($_POST['author_name']) && isset($_POST['author_dept_id'])){
				...
				foreach($authors_id as $author_id){
					...
					$department_id = array_shift($departments);
					$author_name = $database->escape_value(array_shift(trim_explode(ucwords(array_shift($authors)))));
					$department_id = $database->escape_value($department_id);

					if(($author_name != $author->fullname) || ($department_id != $author->dept_id)){
						//UPDATE author SET fullname="Christian Luceno Garillo Sorronda", dept_id="1050" WHERE fullname="Christian Luceno Garillo Sor" and dept_id="1051"
						$sql = "UPDATE author ";
						$sql .= "SET fullname='$author_name', dept_id='$department_id' ";
						$sql .= "WHERE fullname='$author->fullname' and dept_id='$author->dept_id' ";
						$database->query($sql);
						...
						<div class="box-body">
							...
							?>
								<div class="form-group"  id="add_author" >
									<input type="text" name="author_name[]" class="new_author_entry" pattern="[A-Za-z ]{6,}" title="Please don't put any special characters on the name and it's at least 6 letters long." style="width:60%;" placeholder="FirstName MidName LastName" value='<?php echo $author->fullname; ?>' required <?php if(!$editable){ echo " disabled";} ?> />

									<select name="author_dept_id[]" class="new_author_entry" style="width:39%;text-align:center;" required <?php if(!$editable){ echo " disabled";} ?> >
										<option selected disabled>-- Please Select A Department --</option> 
										<?php  
										foreach($departments as $department){
											echo "<option value='{$department->id}' ";
											echo ($author->dept_id==$department->id) ? "selected" : "";	
					...
					<div class="box box-danger">
						...
						<div class="box-body">
							<div class="callout callout-danger"> 
								...
								Only Abstract, Keyword, Category, Document file, Thesis Status and PDF Status are alterable. The rest cannot be changed after 12 hours it was added.
								</p>
								<br />
								<p>
